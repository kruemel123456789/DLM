...
def main():
    ...
    RMS = keras.optimizers.RMSprop(lr=lr, rho = 0.9, epsilon=1e-08, decay = 0.0)
    Adag = keras.optimizers.Adagrad(lr=lr, epsilon=1e-08, decay = 0.0)
    Adad = keras.optimizers.Adadelta(lr=lr, rho = 0.95, epsilon=1e-08, decay = 0.0)
    Ada = keras.optimizers.Adam(lr=lr, beta_1=0.9 , beta_2=0.999, epsilon=1e-08, decay = 0.0)
    opti = Adad
    ...
    # Callbacks
    tensorboard = TensorBoard(log_dir='./logs/7')
    ...
    print("Test loss: {} \nTest accuracy: {}%".format(score[0], 100 * score[1]))
